...
DynamodbFactory.config = function (o) {
  if (o.hasOwnProperty("empty_string_replace_as")) {
    ...
    DynamodbFactory.util.config.empty_string_replace_as =
      o.empty_string_replace_as;
  }

  if (o.hasOwnProperty("stringset_parse_as_set"))
    DynamodbFactory.util.config.stringset_parse_as_set =
      o.stringset_parse_as_set;

  if (o.hasOwnProperty("numberset_parse_as_set"))
    DynamodbFactory.util.config.numberset_parse_as_set =
      o.numberset_parse_as_set;

  if (o.hasOwnProperty("binaryset_parse_as_set"))
    DynamodbFactory.util.config.binaryset_parse_as_set =
      o.binaryset_parse_as_set;
};

var Dynamo = require("@aws-sdk/client-dynamodb");
...
function DynamoDB($config) {
  ...
  if (
    ...
    ($config.config || {}).hasOwnProperty("dynamoDbCrc32")
  ) {
...
DynamoDB.prototype.N = function (data) {
  ...
  throw new Error(
    "N: argument should be a Number or String that converts to a Number"
  );
};
...
DynamoDB.prototype.NS = function (data) {
  if (Array.isArray(data)) {
    var $to_ret = [];
    return new DynamodbFactory.util.Raw({
      NS: data.map(function (el, idx) {
        return el.toString();
      }),
    });
  }
...
DynamoDB.prototype.add = function (data, datatype) {
  ...
  if (typeof datatype == "string") {
    switch (datatype) {
      ...
      case "S":
        throw new Error(
          "ADD action is not supported for the type: " + datatype
        );
        break;
      ...
      default:
        throw new Error(
          "ADD action is not supported by aws-dynamodb for type: " + datatype
        );
        break;
    }
    return;
  ...
  throw new Error(
    "ADD action is not supported by aws-dynamodb for type: " + typeof data
  );
};
...
DynamoDB.prototype.del = function (data, datatype) {
  ...
  if (typeof datatype == "string") {
    switch (datatype) {
      case "NS":
        return this.del(this.NS(data));
        break;
      case "SS":
        return this.del(this.SS(data));
        break;
      ...
      case "L":
        throw new Error(
          "DELETE action with value is not supported for the type: " + datatype
        );
        break;
      ...
      default:
        throw new Error(
          "DELETE action is not supported by aws-dynamodb for type: " + datatype
        );
        break;
    }
    return;
  ...
  throw new Error(
    "DELETE action is not supported by aws-dynamodb for type: " + typeof data
  );
};
...
DynamoDB.prototype.addTableSchema = function ($schema) {
  if (typeof $schema !== "object")
    throw new Error(
      "[AWSPILOT] Invalid parameter, schema must be Array of Objects or Object"
    );
...
DynamoDB.prototype.schema = function ($schemas) {
  ...
  if (typeof $schemas !== "object")
    throw new Error(
      "[AWSPILOT] Invalid parameter, schema must be Array or Object"
    );
...
DynamoDB.prototype.transact = function () {
  var re = this.return_explain;
  this.return_explain = false;
  return new Transact(this.client, {
    events: this.events,
    describeTables: this.describeTables,
    return_explain: re,
  });
    ...
    function (err, raw) {
      ...
      if (!raw.hasOwnProperty("Table"))
        return callback.apply(this, [
          { errorMessage: "Invalid data. No Table Property in describeTable" },
        ]);
...
Request.prototype.select = function () {
  if (
    arguments.length === 1 &&
    arguments[0] === DynamoDB.prototype.ALL_ATTRIBUTES
  ) {
  ...
  if (
    arguments.length === 1 &&
    arguments[0] === DynamoDB.prototype.ALL_PROJECTED_ATTRIBUTES
  ) {
...
Request.prototype.consistentRead = function ($value) {
  ...
  if ($value) this.ConsistentRead = true;
  else this.ConsistentRead = false;
...
Request.prototype.ReturnConsumedCapacity =
  Request.prototype.return_consumed_capacity;
  ...
  this.describeTable(this.tableName, function (err, data) {
    ...
    this.routeCall("putItem", $thisQuery, true, function (err, data) {
      ...
      typeof callback !== "function"
        ...
        : callback.apply(this, [
            err,
            DynamodbFactory.util.normalizeItem(data.Attributes || {}),
            data,
          ]);
    });
  });
...
Request.prototype.replace = function (item, callback) {
  ...
  if (typeof callback !== "function") {
    return new Promise(function (fullfill, reject) {
      $this.describeTable($this.tableName, function (err, data) {
        ...
        for (var i in data.Table.KeySchema) {
          $this
            .if(data.Table.KeySchema[i].AttributeName)
            .eq(item[data.Table.KeySchema[i].AttributeName]);
        }
  ...
  this.describeTable(this.tableName, function (err, data) {
    ...
    for (var i in data.Table.KeySchema) {
      this.if(data.Table.KeySchema[i].AttributeName).eq(
        item[data.Table.KeySchema[i].AttributeName]
      );
    }
...
Request.prototype.update = function ($attrz, callback, $action) {
  ...
  if (typeof callback !== "function") {
    return new Promise(function (fullfill, reject) {
      $this.describeTable($this.tableName, function (err, data) {
        ...
        for (var i in data.Table.KeySchema) {
          if (
            !$this.whereKey.hasOwnProperty(
              data.Table.KeySchema[i].AttributeName
            )
          ) {
            ...
            return reject({
              message:
                "Uncaught ValidationException: Missing value for Attribute '" +
                data.Table.KeySchema[i].AttributeName +
                "' in .where()",
            });
          } else {
            $this.if(data.Table.KeySchema[i].AttributeName).eq(
              DynamodbFactory.util.normalizeItem({
                key: $this.whereKey[data.Table.KeySchema[i].AttributeName],
              }).key
            );
          }
        }
  ...
  this.describeTable(this.tableName, function (err, data) {
    ...
    for (var i in data.Table.KeySchema) {
      if (
        !this.whereKey.hasOwnProperty(data.Table.KeySchema[i].AttributeName)
      ) {
        // aws will throw: Uncaught ValidationException: The provided key element does not match the schema
        // we're throwing a more understandable error
        typeof callback !== "function"
...
Request.prototype.insert_or_update = function (params, callback, $action) {
  ...
  if (typeof callback !== "function") {
    return new Promise(function (fullfill, reject) {
      $this.describeTable($this.tableName, function (err, data) {
        ...
        for (var i in data.Table.KeySchema) {
          $this
            .where(data.Table.KeySchema[i].AttributeName)
            .eq($attrz[data.Table.KeySchema[i].AttributeName]);
  ...
  this.describeTable(this.tableName, function (err, data) {
    ...
    // extract the hash/range keys
    for (var i in data.Table.KeySchema) {
      this.where(data.Table.KeySchema[i].AttributeName).eq(
        $attrz[data.Table.KeySchema[i].AttributeName]
      );
      delete $attrz[data.Table.KeySchema[i].AttributeName];
    }
    var $to_update = {};
...
Request.prototype.get = function (callback) {
  ...
  this.routeCall("getItem", $thisQuery, true, function (err, data) {
    ...
    typeof callback !== "function"
      ...
      : callback.apply(this, [
          err,
          DynamodbFactory.util.parse({ M: data.Item || {} }),
          data,
        ]);
  });
};
...
Request.prototype.query = function (callback) {
  ...
  if (typeof callback !== "function") {
    return new Promise(function (fullfill, reject) {
      $this.routeCall("query", $thisQuery, true, function (err, data) {
        ...
        fullfill(
          DynamodbFactory.util.parse({
            L: (data.Items || []).map(function (item) {
              return { M: item };
            }),
          })
        );
  ...
  this.routeCall("query", $thisQuery, true, function (err, data) {
    ...
    this.LastEvaluatedKey =
      data.LastEvaluatedKey === undefined ? null : data.LastEvaluatedKey;
...
Request.prototype.sql = function (sql, callback) {
  ...
  if (sqp.length > 1)
    return callback({
      errorCode: "UNSUPPORTED_MULTIQUERY",
      errorMessage: "[AWSPILOT] Multiple queries not supported, yet!",
    });
      ...
      switch (sqp.statement) {
        ...
        case "INSERT":
          $this.describeTable(sqp.dynamodb.TableName, function (err, data) {
            ...
            sqp.dynamodb.Expected = DynamodbFactory.util.buildExpected(
              $this.ifFilter
            );
        ...
        case "UPDATE":
          $this.describeTable(sqp.dynamodb.TableName, function (err, data) {
            ...
            if (
              Object.keys(sqp.dynamodb.Expected).length !==
              Object.keys(data.Table.KeySchema).length
            )
              return reject({ errorCode: "WHERE_SCHEMA_INVALID" });

            for (var i in data.Table.KeySchema) {
              if (
                !sqp.dynamodb.Expected.hasOwnProperty(
                  data.Table.KeySchema[i].AttributeName
                )
              )
                return reject({ errorCode: "WHERE_SCHEMA_INVALID" });
  ...
  switch (sqp.statement) {
    case "DESCRIBE_TABLE":
      ...
      this.routeCall(sqp.operation, sqp.dynamodb, true, function (err, data) {
        ...
        typeof callback !== "function"
          ? null
          : callback.apply(this, [err, data.Table, data]);
      });
      break;
    ...
    case "SHOW_TABLES":
      ...
      this.routeCall(sqp.operation, sqp.dynamodb, true, function (err, data) {
        ...
        typeof callback !== "function"
          ? null
          : callback.apply(this, [err, data.TableNames, data]);
      });
      break;
    ...
    case "CREATE_TABLE":
      ...
      this.routeCall(sqp.operation, sqp.dynamodb, true, function (err, data) {
        ...
        typeof callback !== "function"
          ? null
          : callback.apply(this, [err, data.TableDescription, data]);
      });
      break;
      ...
      (function recursive_call($lastKey) {
        ...
        $this.routeCall("scan", sqp.dynamodb, true, function (err, data) {
          ...
          data.Items.map(function (d) {
            inStream.push(
              "INSERT INTO `" +
                sqp.dynamodb.TableName +
                "` VALUES (" +
                DynamodbFactory.util.toSQLJSON(d) +
                ")\n"
            );
          });
...
Request.prototype.compare = function ($comparison, $value, $value2) {
  ...
  if (this.pendingIf !== null) {
    if ($comparison == "EQ") {
      this.ifFilter[this.pendingIf] = new DynamodbFactory.util.Raw({
        Exists: true,
        Value: DynamodbFactory.util.stringify($value),
      });
    } else {
...
Request.prototype.registerExpressionAttributeName = function (item, ALLOW_DOT) {
  ...
  if (!ALLOW_DOT)
    return DynamodbFactory.util
      .expression_name_split(item)
      .map(function (original_attName) {
        var attName = original_attName
          .split("-")
          .join("_minus_")
          .split(".")
          .join("_dot_"); // "-","." not allowed
        var attSpecialName = "#" + attName;

        if (attName.indexOf("[") !== -1) {
          attSpecialName = attName
            .split("[")
            .map(function (v) {
...
Request.prototype.registerExpressionAttributeValue = function (
  original_attName,
  value
) {
  ...
  var attName = original_attName
    ...
    .join("_minus_")
    .split('"')
    .join("_quote_"); // "-" not allowed

  var attNameValue =
    ":" +
    attName.split(".").join("_").split("[").join("_idx_").split("]").join("");

  var attNameValueVersion = 1;
  while (
    this.ExpressionAttributeValues.hasOwnProperty(
      attNameValue + "_v" + attNameValueVersion
    )
  )
    attNameValueVersion++;

  this.ExpressionAttributeValues[attNameValue + "_v" + attNameValueVersion] =
    DynamodbFactory.util.stringify(value);
...
Request.prototype.buildKeyConditionExpression = function (idx) {
  var $this = this;
  var ret = [];
  this.KeyConditionExpression = Object.keys(this.whereKey)
    .map(function (key) {
      return (
        $this.registerExpressionAttributeName(key, true) +
        " " +
        "=" +
        " " +
        $this.registerExpressionAttributeValue(
          key,
          DynamodbFactory.util.normalizeItem({ value: $this.whereKey[key] })
            .value,
          true
        )
      );
...
Request.prototype.buildFilterExpression = function (idx) {
  ...
  var ret = [];
  this.FilterExpression = this.whereFilterExpression
    .map(function (whereFilter) {
      ...
      switch (filterOperators[whereFilter.operator]) {
        ...
        case ">=":
          return (
            $this.registerExpressionAttributeName(whereFilter.attribute) +
            " " +
            filterOperators[whereFilter.operator] +
            " " +
            $this.registerExpressionAttributeValue(
              whereFilter.attribute,
              whereFilter.value
            )
          );
          break;
        ...
        case "BETWEEN":
          return (
            ...
            " BETWEEN " +
            $this.registerExpressionAttributeValue(
              whereFilter.attribute + "_1",
              whereFilter.value
            ) +
            " AND " +
            $this.registerExpressionAttributeValue(
              whereFilter.attribute + "_2",
              whereFilter.value2
            )
          );
          break;
        ...
        case "IN":
          return (
            ...
            whereFilter.value
              .map(function (v, idx) {
                return $this.registerExpressionAttributeValue(
                  whereFilter.attribute + "_" + idx,
                  v
                );
              })
              .join(",") +
